...
class SI:
    ...
    def create_schema_variant_qualification_func(self, schema_id, schema_variant_id, name, code, display_name=None, description=None):
        if not self.change_set_id:
            raise Exception("change set must be created first")
        ...
        ret = requests.post(
            f"{self.base_url}/v1/w/{self.session.workspace_id}/change-sets/{self.change_set_id}/schemas/{schema_id}/variant/{schema_variant_id}/funcs/qualification",
            headers=headers,
            json=func_data,
        )
        ...
        if ret.ok:
            ...
            raise Exception(
                f"Failed to create schema variant qualification func: {ret.status_code} - {ret.text}"
            )
